    ...
    return zn.Controller('product',{
        methods: {
            searchProduct: {
                ...
                value: function (request, response, chain){
                    ...
                    _values.forEach(function (value, index){
                        if(value){
                            _like.push("zn_title like '%"+value+"%'");
                        }
                    });
                    if(_data.vars){
            ...
            getAllTypes: {
                method: 'GET/POST',
                value: function (request, response, chain){
                    this.query("select * from zn_auction_product_type;")
                        .then(function (data){
                            response.success(data);
                        }.bind(this), function (error){
                            response.error(error.message);
            ...
            getTypes: {
                ...
                value: function (request, response, chain){
                    var _sql = "select * from zn_auction_product_type where zn_tree_pid={0};".format(request.getValue('pid'));
            ...
            pagingProductsByType: {
                ...
                value: function (request, response, chain){
                    this.query(zn.sql.paging(zn.extend({
                        table: 'zn_auction_product',
                        fields: '*',
                        where: 'typeId={0} and status<>0 order by zn_create_time desc, endTime desc, beginTime desc'.format(request.getInt('typeId'))
